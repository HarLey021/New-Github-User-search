...
const InfoBox: React.FC<InfoProps> = ({ darkMode, userInfo, setUserInfo }) => {
  useEffect(() => {
  ...
  const isoDate: string = userInfo?.created_at;
  const date: Date = new Date(isoDate);

  const formatted: string = date.toLocaleDateString("en-GB", {
    day: "2-digit",
    month: "short",
    year: "numeric",
  });
  ...
  return (
    <>
      <div
        className={`w-full h-auto pt-[32px] pl-[24px] pr-[24px] pb-[48px] rounded-[15px] tablet:p-[40px] desktop:flex desktop:p-[48px] 
         ${
           darkMode
             ? "bg-content-dark text-white"
             : "bg-content-light text-grey"
         }
        `}
      >
        <img
          src={userInfo?.avatar_url}
          className="hidden desktop:block desktop:w-[117px] desktop:h-[117px] rounded-[117px] desktop:mr-[37px]"
        />
        <div>
          <div className="w-full flex items-center mb-[33px] tablet:mb-[24px] desktop:mb-[20px]">
            <img
              src={userInfo?.avatar_url}
              className="w-[70px] h-[70px] mr-[20px] tablet:w-[117px] tablet:h-[117px] rounded-[117px] tablet:mr-[41px] desktop:hidden"
            />
            <div className="desktop:flex desktop:flex-row">
              <div className="desktop:mr-[138px]">
                <h3
                  className={`text-[16px] font-bold tablet:text-[26px] tablet:mb-[2px] 
                ${darkMode ? "text-white" : "text-dark"}`}
                >
                  {userInfo?.name}
                </h3>
                <h6 className="text-blue text-[13px] font-normal mb-[6px] tablet:text-[16px] tablet:mb-[4px]">
                  {`@${userInfo?.login}`}
                </h6>
              </div>
              <h6 className="text-[13px] font-normal tablet:text-[15px] desktop:mt-[8px]">
                {`Joined ${formatted} `}
              </h6>
            </div>
          </div>

          <p className="text-[13px] font-normal leading-[25px] mb-[23px] tablet:text-[15px] tablet:mb-[32px]">
            {userInfo?.bio ? userInfo.bio : "This profile has no bio"}
          </p>

          <div
            className={`w-full h-[85px] rounded-[10px] pt-[18px] pr-[14px] pb-[19px] pl-[15px] flex justify-between mb-[24px] tablet:px-[32px] tablet:py-[16px] tablet:mb-[30px] desktop:mb-[37px]
            ${darkMode ? "bg-background-dark" : "bg-background-light"}`}
          >
            <div className="flex flex-col items-center tablet:items-start">
              <h6 className="text-[11px] font-normal mb-[8px] tablet:text-[13px] tablet:mb-0">
                Repos
              </h6>
              <h4
                className={`text-[16px] font-bold tablet:text-[22px] 
                ${darkMode ? "text-white" : "text-dark"}`}
              >
          ...
          <div className="tablet:flex tablet:justify-between">
            <div className="tablet:mr-[30px]">
              <div className="flex gap-[19px] items-center mb-[16px] ">
                <svg
                ...
                <div className="max-w-[200px]">
                  <h6 className="text-[13px] font-normal overflow-hidden break-words tablet:text-[15px] ">
              ...
              <div className="flex gap-[13px] items-center mb-[16px]">
                ...
                <div className="max-w-[200px]">
                  <a
                    ...
                    className="text-[13px] font-normal tablet:text-[15px] overflow-hidden break-words desktop:cursor-pointer"
                  >
                    {userInfo?.html_url ? userInfo.html_url : "Not Available"}
                  </a>
                </div>
              </div>
            </div>
            ...
            <div>
              <div className="flex gap-[13px] items-center mb-[16px]">
                ...
                <div className="max-w-[200px]">
                  <a className="text-[13px] font-normal tablet:text-[15px] overflow-hidden break-words">
